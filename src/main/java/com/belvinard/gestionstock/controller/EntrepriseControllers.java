...
public class EntrepriseControllers {
        ...
        @GetMapping("/all")
        @Operation(summary = "PUBLIC: Retourne la liste des entreprises", description = "Retourne la liste complète des entreprises")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Liste des entreprises récupérée avec succès", content = @Content(mediaType = "application/json", schema = @Schema(implementation = EntrepriseResponse.class)))
        })
        ...
        @GetMapping("/{id}")
        @Operation(summary = "PUBLIC: Récupère une entreprise par son ID", description = "Cette méthode permet de récupérer une entreprise en fonction de son ID")
        @ApiResponses(value = {
        ...
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Supprimer une entreprise par son ID", description = "Cette méthode permet de supprimer une entreprise en fonction de son ID")
        @ApiResponses(value = {
        ...
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Mettre à jour l'image de l'entreprise", description = "Uploader une nouvelle image pour l'entreprise via MinIO")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Image mise à jour avec succès", content = @Content(schema = @Schema(implementation = EntrepriseDTO.class))),
        ...
        public ResponseEntity<EntrepriseDTO> updateEntrepriseImage(
                        ...
                        @Parameter(description = "Fichier image", required = true) @RequestParam("image") MultipartFile image)
                        throws IOException {
        ...
        @Operation(summary = "Obtenir l'URL pré-signée de l'image d'une entreprise", description = "Retourne une URL pré-signée pour accéder à l'image de l'entreprise")
        @ApiResponses(value = {
