...
public class LigneCommandeClientController {
        ...
        @Operation(summary = "Récupérer toutes les lignes de commande client (ADMIN ou SALES_MANAGER)", description = "Retourne la liste de toutes les lignes de commande client avec les détails enrichis")
        @ApiResponses(value = {
        ...
        @GetMapping("/{ligneId}")
        @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_SALES_MANAGER')")
        @Operation(summary = "Récupérer une ligne de commande client par ID (ADMIN ou SALES_MANAGER)", description = "Retourne une ligne de commande client en fonction de son identifiant unique.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<LigneCommandeClientDTO> getLigneCommandeClient(
                        @Parameter(description = "ID de la ligne de commande client à récupérer", required = true) @PathVariable Long ligneId) {

                LigneCommandeClientDTO ligneCommandeClientDTO = ligneCommandeClientService
                                .getLigneCommandeClientById(ligneId);
                return ResponseEntity.ok(ligneCommandeClientDTO);
        }

        @PutMapping("/{ligneId}")
        @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_SALES_MANAGER')")
        @Operation(summary = "Mettre à jour une ligne de commande client (ADMIN ou SALES_MANAGER)", description = "Impossible si la commande est déjà livrée.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Ligne mise à jour", content = @Content(schema = @Schema(implementation = LigneCommandeClientDTO.class))),
        ...
        public ResponseEntity<LigneCommandeClientDTO> updateLigneCommandeClient(
                        ...
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Données mises à jour pour la ligne de commande", required = true, content = @Content(schema = @Schema(implementation = LigneCommandeClientDTO.class))) @RequestBody LigneCommandeClientDTO ligneDTO) {

                LigneCommandeClientDTO updatedLigne = ligneCommandeClientService.updateLigneCommandeClient(ligneId,
                                ligneDTO);
        ...
        @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_SALES_MANAGER')")
        @Operation(summary = "Supprimer une ligne de commande client (ADMIN ou SALES_MANAGER)", description = "Cette opération permet de supprimer une ligne de commande client sauf si la commande est déjà livrée.")
        @ApiResponses(value = {
        ...
        @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_SALES_MANAGER')")
        @Operation(summary = "Historique des commandes d'un article (ADMIN ou SALES_MANAGER)", description = "Récupère la liste des lignes de commande client pour un article donné")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<LigneCommandeClientDTO>> getHistoriqueCommandeClient(
                        @Parameter(description = "ID de l'article concerné", example = "2") @PathVariable Long idArticle) {
                List<LigneCommandeClientDTO> historique = ligneCommandeClientService
                                .findHistoriqueCommandeClient(idArticle);
